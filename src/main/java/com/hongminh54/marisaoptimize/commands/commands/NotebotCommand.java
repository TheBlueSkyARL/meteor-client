...
import com.hongminh54.marisaoptimize.commands.arguments.NotebotSongArgumentType;
...
import com.hongminh54.marisaoptimize.systems.modules.misc.Notebot;
import com.hongminh54.marisaoptimize.utils.notebot.song.Note;
...
public class NotebotCommand extends Command {
    ...
    public void build(LiteralArgumentBuilder<CommandSource> builder) {
        ...
        builder.then(literal("record").then(literal("start").executes(ctx -> {
            ticks = -1;
            song.clear();
            MarisaOptimize.EVENT_BUS.subscribe(this);
        ...
        builder.then(literal("record").then(literal("save").then(argument("name", StringArgumentType.greedyString()).executes(ctx -> {
            ...
            Path notebotFolder = MarisaOptimize.FOLDER.toPath().resolve("notebot");
