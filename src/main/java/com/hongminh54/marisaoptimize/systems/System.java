...
public abstract class System<T> implements ISerializable<T> {
    ...
    public System(String name) {
        ...
        if (name != null) {
            this.file = new File(MarisaOptimize.FOLDER, name + ".nbt");
    ...
    public void save(File folder) {
        ...
        try {
            File tempFile = File.createTempFile(MarisaOptimize.MOD_ID, file.getName());
            ...
            MarisaOptimize.LOG.error("Error saving {}. Possibly corrupted?", this.name, e);
        }
    }
        ...
        try {
            ...
            if (file.exists()) {
                try {
                    ...
                    MarisaOptimize.LOG.error("Error loading {}. Possibly corrupted?", this.name, e);
                    MarisaOptimize.LOG.info("Saved settings backup to '{}'.", backup);
                }
            }
        } catch (IOException e) {
            MarisaOptimize.LOG.error("Error loading {}. Possibly corrupted?", this.name, e);
