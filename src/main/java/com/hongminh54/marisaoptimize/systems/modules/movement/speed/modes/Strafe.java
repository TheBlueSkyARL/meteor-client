...
package com.hongminh54.marisaoptimize.systems.modules.movement.speed.modes;
...
public class Strafe extends SpeedMode {
    ...
    public static Vector2d transformStrafe(double speed) {
        float forward = Math.signum(MarisaOptimize.mc.player.input.getMovementInput().y);
        float side = Math.signum(MarisaOptimize.mc.player.input.getMovementInput().x);
        float yaw = MarisaOptimize.mc.player.getLerpedYaw(MarisaOptimize.mc.getRenderTickCounter().getTickProgress(true));
