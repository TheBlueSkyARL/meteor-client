...
public abstract class Module implements ISerializable<Module>, Comparable<Module> {
    ...
    public Module(Category category, String name, String description, String... aliases) {
        if (name.contains(" ")) MarisaOptimize.LOG.warn("Module '{}' contains invalid characters in its name making it incompatible with Meteor Client commands.", name);
    ...
    public void toggle() {
        if (!active) {
            ...
            if (runInMainMenu || Utils.canUpdate()) {
                if (autoSubscribe) MarisaOptimize.EVENT_BUS.subscribe(this);
                onActivate();
            }
        }
        else {
            if (runInMainMenu || Utils.canUpdate()) {
                if (autoSubscribe) MarisaOptimize.EVENT_BUS.unsubscribe(this);
                onDeactivate();
            }
